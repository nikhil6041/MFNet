...
parser.add_argument('--loss_fun',default = 'quadtriplet', required=True,type=str,
                    help="Loss function to use a) triplet or b) quadtriplet ?")
...
parser.add_argument('--alpha1', default=0.5, required=True,type=float, 
                    help='alpha1 (default: 0.5)')
parser.add_argument('--alpha2', default=0.5, required=True,type=float, 
                    help='alpha2 (default: 0.5)')
parser.add_argument('--alpha3', default=0.5, required=True,type=float, 
                    help='alpha3 (default: 0.5)')
parser.add_argument('--alpha4', default=0.5, required=True,type=float, 
                    help='alpha4 (default: 0.5)')
...
parser.add_argument('--save-dir',type = str, default='D:\Academics\HonsProject1',
...
parser.add_argument('--pretrain_checkpoint',default='casia-webface' ,type=str,
                    help='Pretrained checkpoints casia-webface or vggface2')
parser.add_argument('--fc-only',default= False, action='store_true',
                    help='Train fc only')
parser.add_argument('--except-fc',default=False, action='store_true',
                    help='Train the base except fc layer')
...
parser.add_argument('--load-last',default=False, action='store_true',
                    help='Load the last checkpoint')
parser.add_argument('--continue-step',default=False, action='store_true',
                    help='resume training from last checkpoint')
parser.add_argument('--train-all', action='store_true',default=True, 
                    help='Train all layers')
parser.add_argument('--last_ckpt_name',required=True,
                    help='Last checkpoint name')
parser.add_argument('--best_ckpt_name',required=True,
...
def main():
    ...
    alpha3 = args['alpha3']
    alpha4 = args['alpha4']
    print(f"Loss function: {loss_fun}")
    ...
    quad_triplet_loss = QuadTripletLoss(alpha1,alpha2,alpha3,alpha4).to(device)
    ...
    for epoch in range(start_epoch, args.num_epochs + start_epoch):
        ...
        train_valid(model, optimizer, quad_triplet_loss, scheduler, epoch, data_loaders, data_size,save_dir)
